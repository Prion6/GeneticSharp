...
namespace GeneticSharp.Domain.UnitTests.Crossovers
{
    ...
    public class AlternatingPositionCrossoverTest
    {
        ...
        public void Cross_ParentWithNoOrderedGenes_Exception()
        {
            ...
            chromosome1.ReplaceGenes(0, new int[] {8, 2, 3, 4, 6, 5, 7, 1});

            // 3 7 5 1 6 8 2 4
            var chromosome2 = Substitute.For<ChromosomeBase<int>>(8);
            chromosome2.ReplaceGenes(0, new int[] {1, 2, 3, 4, 5, 5, 6, 7});
        ...
        public void Cross_DocumentationSample_Child()
        {
            ...
            var child1 = Substitute.For<ChromosomeBase<int>>(8);
            chromosome1.CreateNew().Returns(child1);

            // 3 7 5 1 6 8 2 4
            var chromosome2 = Substitute.For<ChromosomeBase<int>>(8);
            chromosome2.ReplaceGenes(0, new int[] { 3, 7, 5, 1, 6, 8, 2, 4});
            var child2 = Substitute.For<ChromosomeBase<int>>(8);
            ...
            Assert.AreEqual(3, actualChild.GetGene(1));
            Assert.AreEqual(2, actualChild.GetGene(2));
            Assert.AreEqual(7, actualChild.GetGene(3));
            Assert.AreEqual(5, actualChild.GetGene(4));
            Assert.AreEqual(4, actualChild.GetGene(5));
            Assert.AreEqual(6, actualChild.GetGene(6));
            Assert.AreEqual(8, actualChild.GetGene(7));

            // 3 1 7 2 5 4 6 8
            actualChild = actual[1];
            Assert.AreEqual(8, actualChild.Length);
            Assert.AreEqual(3, actualChild.GetGene(0));
            Assert.AreEqual(1, actualChild.GetGene(1));
            Assert.AreEqual(7, actualChild.GetGene(2));
            Assert.AreEqual(2, actualChild.GetGene(3));
            Assert.AreEqual(5, actualChild.GetGene(4));
            Assert.AreEqual(4, actualChild.GetGene(5));
            Assert.AreEqual(6, actualChild.GetGene(6));
            Assert.AreEqual(8, actualChild.GetGene(7));
